/**
 * SQLite implementation of the data layer.
 *
 * Selected by server/lib/db.js when DATABASE_URL is not set, which is the
 * default for local development. Methods are exported wrapped as async so
 * that callers await the data layer identically whichever backend is live --
...
_Feedback.setMany = db.transaction((userId, entries, at) => {
...
/**
 * better-sqlite3 is synchronous and node-postgres is not. Rather than have
 * every route branch on the backend, the SQLite methods are presented as
 * async too. The work still happens synchronously; only the signature changes.
 */
const asAsync = (obj) => Object.fromEntries(
  Object.entries(obj).map(([key, value]) => [
    key,
    typeof value === 'function' ? async (...args) => value.apply(obj, args) : value,
  ]),
);

export const Users = { ...asAsync(_Users), count: async () => countUsers() };
export const Sessions = asAsync(_Sessions);
export const Feedback = asAsync(_Feedback);
export const Hubs = asAsync(_Hubs);
export const Posts = asAsync(_Posts);
export const Reviews = asAsync(_Reviews);

const countUsers = () => db.prepare('SELECT COUNT(*) AS n FROM users').get().n;

export async function migrate() { migrateSync(); }

/** Run `fn` inside a transaction. Mirrors the Postgres implementation. */
...
/**
 * Bulk inserts, to match the Postgres surface. Here they are simply loops in
 * a transaction -- there is no network round trip to save.
 */
Users.createMany = async (rows) => db.transaction(() => {
  for (const r of rows) {
    if (!_Users.byId(r.id)) _Users.create(r);
    if (r.onboarded ?? true) _Users.markOnboarded(r.id);
  }
})();

Reviews.upsertMany = async (rows) => db.transaction(() => {
  for (const r of rows) _Reviews.upsert(r);
})();

Posts.createMany = async (rows) => db.transaction(() => {
  for (const r of rows) if (!_Posts.byId(r.id)) _Posts.create(r);
})();

Posts.addCommentMany = async (rows) => db.transaction(() => {
  for (const r of rows) if (!_Posts.commentById(r.id)) _Posts.addComment(r);
})();

Posts.voteMany = async (rows) => db.transaction(() => {
  for (const r of rows) _Posts.vote(r.post_id, r.user_id, r.created_at ?? Date.now());
})();

Hubs.joinMany = async (rows) => db.transaction(() => {
  for (const r of rows) _Hubs.join(r.franchise, r.user_id, r.created_at ?? Date.now());
})();
